...
import {
  ...
  downloadAuctioneerExcelRequest,
  downloadAuctioneerPDFRequest,
  fetchAuctionRequest,
  fetchGradeRequest,
  fetchMarkRequest,
...
function ViewDealBookAuctioneer({ modalRight }) {
  ...
  const handleExport = (exportType) => {
    const updatedDealBookSearch = {
      ...DealBookSearch,
      url: "/postauction/dealbook/auctioneer/search",
  ...
  const ViewDealBookList = async () => {
    try {
      const response = await axiosMain.post(
        "/postauction/dealbook/auctioneer/search",
                      ...
                      <tbody>
                        {selectedRowsData.map((data, index) => (
                          <tr key={index}>
                            ...
                            <td>{data.dealTime}</td>
                            <td>
                              {data.dealStatus === 4 ? "Canceled" : ""}
                              {data.dealStatus === 3 ? "Transfared" : ""}
                              {data.dealStatus === 2 ? "Closed" : ""}
                              {data.dealStatus === 0 ? "Pending" : ""}
                              {data.dealStatus === 1 ? "Updated" : ""}
                            </td>
                            <td>{data.withinState}</td>
                            <td>{data.interstate}</td>
                            <td>{data.export}</td>
...
export default ViewDealBookAuctioneer;
